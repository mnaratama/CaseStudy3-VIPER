...
class PortofolioViewSpy: PortofolioViewProtocol {
    ...
    var dataCount = 0
    func displayDataChart(data: [ProtofolioApp.PortofolioPieData]) {
...
class PortofolioRouterSpy: PortofolioRouterProtocol {
    static func createModule(navigateToTransactionData: @escaping (ProtofolioApp.PortofolioPieData) -> Void) -> UIViewController {
        return UIViewController()
    }
    
    var routeToTransactionDataCalled = 0
    func routeToTransactionData(transactionData: ProtofolioApp.PortofolioPieData) {
