...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        self.window = window
        let navigation = UINavigationController(rootViewController: PortofolioRouter.createModule(navigateToTransactionData: { data in
            if let topController = UIApplication.shared.keyWindow?.rootViewController as? UINavigationController {
                let transactionData: [TransactionData] = data.transaction.map {
                    TransactionData(transactionType: data.label, transactionDate: $0.trx_date, amount: $0.nominal)
                }
